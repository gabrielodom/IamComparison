...
###  By Run  ###
# # This script takes 52 seconds for one run and one design point
# whichRun_int <- 10
# where_char <- paste0(
...
# whichFile <- 4
# 3 is easiest (100% DE + 8x SD signal), 4 is hardest (20% DE + 2x SD signal),
#   and 8 is medium (50% DE + 4x SD signal)
...
###  Filenames  ###
geneDE_path <- paste0("GeneExp_synth_", whichDesign, "_Diff.RData")
geneData_path <- paste0("GeneExp_synth_", whichDesign, ".RData")
diffMethyl_path <- paste0("Methyl_synth_", whichDesign, "_Diff.RData")
methylData_path <- paste0("Methyl_synth_", whichDesign, ".RData")
