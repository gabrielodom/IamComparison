...
results_ls <- lapply(designs_char, function(desg){
  ...
  out_ls <- lapply(runs, function(runDir){
    ...
    # summaryDir <- paste0(dataDir, "synthetNMF/supervised_k2-ES/RData/")
    # acc_arr <- readRDS(paste0(summaryDir, runDir, "NMF_acc_arr.RData"))
    # auc_arr <- readRDS(paste0(summaryDir, runDir, "NMF_auc_arr.RData"))
    # f1_arr <- readRDS(paste0(summaryDir, runDir, "NMF_f1_arr.RData"))
    # FPR_arr <- readRDS(paste0(summaryDir, runDir, "NMF_FPR_arr.RData"))
    # kappa_arr <- readRDS(paste0(summaryDir, runDir, "NMF_kappa_arr.RData"))
    # mcc_arr <- readRDS(paste0(summaryDir, runDir, "NMF_mcc_arr.RData"))
    # PW_mcc_arr <- readRDS(paste0(summaryDir, runDir, "NMF_PW_mcc_arr.RData"))
    # TPR_arr <- readRDS(paste0(summaryDir, runDir, "NMF_TPR_arr.RData"))
    # 
    # # These don't really have the information we need.
    # rm(
    #   summaryDir, acc_arr, auc_arr, f1_arr, FPR_arr, kappa_arr, mcc_arr, 
    #   PW_mcc_arr, TPR_arr
    # )
